...
from hrms.decorators import login_required as func_login_required
from hrms.http.response import HRMSRedirect
from hrms.signals import post_generic_delete, pre_generic_delete
...
from hrms_views.generic.cbv.views import HRMSFormView, HRMSListView
...
@method_decorator(login_required, name="dispatch")
class DynamiListView(HRMSListView):
    """
...
@method_decorator(login_required, name="dispatch")
class HRMSDeleteConfirmationView(View):
    """
    ...
    def post(self, *args, **kwargs):
        ...
        return HRMSFormView.HttpResponse(script=script)
...
def export_data(request, *args, **kwargs):
    ...
    model_path = request.GET.get("model")
    if not model_path:
        return HRMSRedirect(request, message=_("No matching query found."))
