...
from hrms.hrms_middlewares import _thread_locals, get_selected_company
from hrms.http.response import HRMSRedirect
from hrms.models import HRMSModel
from hrms.nested_group_by import nested_group_by_queryset
from hrms.signals import post_generic_import, pre_generic_import
from hrms_views import models
from hrms_views.cbv_methods import (  # update_initial_cache,
    assign_related,
    export_xlsx,
...
from hrms_views.related_link_registry import RELATED_VIEW_PARAM, register_detail_view
...
def model_has_audit_tracking(model) -> bool:
    ...
    ``hrms_history`` field every ``HRMSModel`` subclass gets.
    """
    return (
...
class HRMSListView(ListView):
    ...
    template_name: str = "generic/hrms_list_table.html"
    ...
    # Mirrors HRMSNavView.nested_group_by_fields -- List and Nav are
    ...
    def get_context_data(self, **kwargs: Any):
        ...
        if (
            ...
            and issubclass(self.model, HRMSModel)
        ...
        if request and len(nested_fields) >= 1:
            ...
            try:
                ...
                context["nested_fields_active"] = HRMSNavView._resolve_field_labels(
                    nested_fields, self.model, self.model()._meta.get_field
                )
                context["nested_group_by_fields"] = (
                    HRMSNavView._resolve_field_labels(
                ...
                self.template_name = "generic/hrms_list_table.html"
    ...
    def serve_import_sheet(self, request, *args, **kwargs):
        ...
        if not self.import_accessibility():
            ...
            return HRMSFormView.HttpResponse()
...
class HRMSDetailedView(DetailView):
    ...
    # target for `model` (see hrms_views/related_link_registry.py), instead
...
class HRMSTabView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # hrms_tabs.html and a <button> in the theme's override, and this
            ...
            if self.request.method == "GET":
                ...
                for dynamic_tuple in self.dynamic_create_fields:
                    ...
                    form.fields[field].widget.option_template_name = (
                        "hrms_widgets/select_option.html",
                    )
        ...
        for field in fields:
            if isinstance(field, str):
                try:
                    ...
                    # the final field, same resolution hrms/group_by.py
...
class HRMSProfileView(DetailView):
    ...
    # hrms_views/urls.py — see dispatch_profile_tab() below for why this
    ...
    def _register_tabs(cls) -> None:
        ...
        for tab in cls.tabs:
            if not tab.get("url"):
                ...
                HRMSProfileView._tab_view_registry[key] = tab["view"]
...
def dispatch_profile_tab(request, tab_key: str, pk: int, *args, **kwargs):
    ...
    hrms_views/urls.py) that every HRMSProfileView tab (candidate
    profile, employee profile, etc.) resolves through.

    HRMSProfileView used to append a brand new URL pattern to the live
    ...
    in HRMSProfileView._tab_view_registry at request time removes both
    problems: the registry is fully populated before Django serves its first
    request (see HRMSProfileView._register_tabs), the same as it would be
    in every worker process.
    """
    view_func = HRMSProfileView._tab_view_registry.get(tab_key)
