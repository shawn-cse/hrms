...
def get_diff(instance, history_related_name):
    ...
    for pair in pairs:
        ...
        if not diffs:
            continue
        updated_by = (
            HRMSUser.objects.get(id=pair[0].history_user.id).employee_get
    ...
    # simple-history) have no rows — still surface creation from HRMSModel.
    ...
    if instance._meta.model_name == "employeeworkinformation":
        from hrms_audit.models import HistoryTrackingFields
