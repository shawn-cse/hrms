...
def update_payslip_status(request, payslip_id):
    ...
    if not payslip:
        return HRMSRedirect(request, message=_("Payslip not found."))
    if payslip:
...
def view_payslip_pdf(request, payslip_id):
    ...
    if Payslip.objects.filter(id=payslip_id).exists():
        ...
        if (
            ...
            for format_name, format_string in pay_settings.HRMS_DATE_FORMATS.items():
...
def delete_payrollrequest_comment(request, comment_id):
    ...
    comment.delete()
    return HRMSRedirect(request, message=_("Comment deleted successfully!"))
...
def delete_reimbursement_comment_file(request):
    ...
    if not ids:
        return HRMSRedirect(request, message=_("No file IDs provided for deletion."))
    ...
    records.delete()
    return HRMSRedirect(request, message=_("File deleted successfully"))
...
def initial_notice_period(request):
    ...
    if not request.GET.get("notice_period"):
        return HRMSRedirect(request, message=_("required parameter is missing"))
