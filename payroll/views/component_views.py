...
from hrms.methods import dynamic_attr, get_hrms_model_class, get_urlencode
...
def update_allowance(request, allowance_id, **kwargs):
    ...
    is_htmx = request.headers.get("HX-Request") is not None
    if not instance:
        return HRMSRedirect(request, message=_("Allowance not found."))
...
#     ):
#         return return HRMSRedirect(request)
...
def update_deduction(request, deduction_id, **kwargs):
    ...
    is_htmx = request.headers.get("HX-Request") is not None
    if not instance:
        return HRMSRedirect(request, message=_("Deduction not found."))
...
def validate_start_date(request):
    ...
    try:
        ...
        return HRMSRedirect(request, message=_("Invalid Request"))
    ...
    for column_value, column_name in selected_columns:
        ...
        for payslip in payslips:
            ...
            if type(value) == date:
                ...
                for format_name, format_string in settings.HRMS_DATE_FORMATS.items():
...
def add_bonus(request):
    ...
    if payslip_id != "None" and payslip_id:
        ...
        if not instance:
            return HRMSRedirect(request, _("Payslip not found"))
...
def view_installments(request):
    ...
    if not loan:
        return HRMSRedirect(request, message=_("Loan not found."))
...
def reimbursement_individual_view(request, instance_id):
    ...
    if not reimbursement:
        return HRMSRedirect(request, message=_("Reimbursement request not found."))
    requests_ids_json = request.GET.get("instances_ids")
    if requests_ids_json:
        requests_ids = json.loads(requests_ids_json)
