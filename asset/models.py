...
from hrms.models import HRMSModel, upload_path
...
class AssetCategory(HRMSModel):
    ...
    objects = HRMSCompanyManager("company_id")
...
class Asset(HRMSModel):
    ...
    def asset_status_col(self):
        ...
        if self.quantity > 1:
            ...
            label = f"{label}<span class='inline-block border-2 border-solid rounded font-bold text-[0.8rem] px-2 py-1 text-[#2563eb] border-[#2563eb] ms-5' title='{self.available_count} of {self.quantity} Available'>{self.available_count}/{self.quantity}</span>"
            return label
...
class AssetItem(HRMSModel):
    ...
    objects = HRMSCompanyManager("asset_id__asset_category_id__company_id")
