...
def update_stage_order(request, pk):
    ...
    if not recruitment:
        return HRMSRedirect(request, message=_("Recruitment not found."))
...
def candidate_update(request, obj_id):
    ...
    if not candidate:
        return HRMSRedirect(request, message=_("Candidate not found."))
    ...
    for cand_id in candidates:
        ...
        try:
            ...
            if company and company.icon and os.path.exists(company.icon.path):
                ...
                image_path = finders.find("images/ui/hrms-sticker-round.png")
...
def user_creation(request, token):
    ...
    try:
        ...
        user = HRMSUser.objects.filter(username=candidate.email).first()
...
def employee_creation(request, token):
    ...
    if user is None:
        ...
        user = HRMSUser.objects.filter(username=candidate.email).first() or user
...
def employee_bank_details(request, token):
    ...
    if not onboarding_portal:
        return HRMSRedirect(request, message=_("Onboarding portal not found."))

    user = HRMSUser.objects.filter(
...
def get_status(request, task_id):
    ...
    if not cand_stage_obj or not onboarding_task or not candidate or not candidate_task:
        return HRMSRedirect(request, message=_("Object not found."))
...
def change_task_status(request):
    ...
    if not task_id or not status:
        return HRMSRedirect(request, message=_("Task ID or status is missing"))
    candidate_task = CandidateTask.find(task_id)
    if not candidate_task:
        return HRMSRedirect(request, message=_("Candidate task not found"))
    if status in [
        "todo",
        "scheduled",
        "ongoing",
