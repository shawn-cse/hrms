...
def create_task_in_project(request, project_id):
    ...
    if not project:
        return HRMSRedirect(request, message=_("Project not found"))
...
def update_timesheet_task(request, timesheet_id):
    ...
    if not timesheet:
        return HRMSRedirect(request, message=_("Timesheet not found"))
...
def delete_project_stage(request, stage_id):
    ...
    stage = ProjectStage.objects.filter(id=stage_id).first()
    if not stage:
        return HRMSRedirect(request, message=_("Project stage not found"))
...
def time_sheet_delete(request, time_sheet_id):
    ...
    Returns:
        HRMSRedirect: A redirect response to the timesheet view page.
    """
