...
A complete, step-by-step guide to running HRMS using Docker. Covers development setup, production deployment, customization, troubleshooting, and maintenance.
...
Get HRMS running in under 5 minutes:
...
| Variable | Default | Description |
|----------|---------|-------------|
| `POSTGRES_DB` | `hrms_db` | Database name |
| `POSTGRES_USER` | `hrms_user` | Database user |
| `POSTGRES_PASSWORD` | `hrms_pass` | Database password — **change for production** |
...
docker compose exec web env GUNICORN_RELOAD=true gunicorn hrms.wsgi:application --config docker/gunicorn.conf.py
...
# Redis CLI
docker compose exec redis redis-cli -a hrms_pass
```
...
With `DEBUG=False` (or `HRMS_ENV=production`), Django refuses known-insecure secrets.
...
From the HRMS login page, click "Load Demo Data" to populate the system with sample employees, departments, and other test data.
...
HRMS supports AWS S3 and Google Cloud Storage. Add to your environment:
...
DEFAULT_FILE_STORAGE=hrms.hrms_backends.PrivateMediaStorage
```
...
DEFAULT_FILE_STORAGE=hrms.hrms_backends_gcp.PrivateMediaStorage
```
...
```yaml
redis:
  command: redis-server --appendonly yes --requirepass hrms_pass --maxmemory 256mb --maxmemory-policy allkeys-lru
```
...
```bash
# SQL backup
docker compose exec db pg_dump -U hrms_user hrms_db > backup_$(date +%Y%m%d_%H%M%S).sql

# Compressed backup
docker compose exec db pg_dump -U hrms_user -F c hrms_db > backup_$(date +%Y%m%d).dump
```
...
```bash
# From SQL file
cat backup.sql | docker compose exec -T db psql -U hrms_user -d hrms_db

# From compressed dump
docker compose exec -T db pg_restore -U hrms_user -d hrms_db --clean < backup.dump
```
...
```bash
# Backup media volume
docker run --rm -v hrms_media:/data -v $(pwd):/backup alpine tar czf /backup/media_backup.tar.gz -C /data .

# Restore media volume
docker run --rm -v hrms_media:/data -v $(pwd):/backup alpine tar xzf /backup/media_backup.tar.gz -C /data
```
...
2. Check database connections: `docker compose exec db psql -U hrms_user -c "SELECT count(*) FROM pg_stat_activity;"`
3. Check Redis memory: `docker compose exec redis redis-cli -a hrms_pass INFO memory`
