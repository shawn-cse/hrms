...
from hrms.menu.settings_menu import get_settings_menu
...
def _get_employee_of_user(user):
    ...
    """
    if not hasattr(user, "_hrms_employee_cache"):
        user._hrms_employee_cache = Employee.objects.filter(
            employee_user_id=user
        ).first()
    return user._hrms_employee_cache
...
def is_manager_of(user, instance, field_name="employee_id"):
    ...
    if not hasattr(user, "_hrms_is_manager_of_cache"):
        user._hrms_is_manager_of_cache = {}
    cache = user._hrms_is_manager_of_cache
...
def is_leave_approval_manager(user):
    ...
    """
    if hasattr(user, "_hrms_is_leave_approval_manager_cache"):
        return user._hrms_is_leave_approval_manager_cache
    employee = _get_employee_of_user(user)
    manager = (
    ...
    user._hrms_is_leave_approval_manager_cache = manager
    return manager
...
def general_section_main(context):
    ...
    return any(
        [
            ...
            user.has_perm("hrms_audit.view_accountblockunblock"),
            ...
            user.has_perm("hrms_audit.view_historytrackingfields"),
            ...
            user.has_perm("hrms_audit.view_audittag"),
            user.has_perm("base.view_dynamicemailconfiguration"),
            user.has_perm("hrms_backup.view_googledrivebackup"),
        ]
    )
...
def show_section(context):
    ...
    return any(
        [
            ...
            user.has_perm("hrms_audit.view_audittag"),
            user.has_perm("hrms_backup.view_googledrivebackup"),
            user.has_perm("hrms_ldap.add_ldapsettings"),
            user.has_perm("hrms_ldap.update_ldapsettings"),
            user.has_perm("employee.view_actiontype"),
            user.has_perm("base.view_tags"),
            ...
            user.has_perm("hrms_ldap.add_ldapsettings"),
            user.has_perm("hrms_ldap.update_ldapsettings"),
            user.has_perm("hrms_meet.view_googlecloudcredential"),
            user.has_perm("whatsapp.add_whatsappcredientials"),
            user.has_perm("hrms_theme.view_hrmscolortheme"),
        ]
    )
...
def settings_search_index():
    ...
    from hrms.menu.settings_menu import settings_registry
