...
def _resolve_company_theme(company_id):
    if company_id is not None and company_id != "all":
        ...
        if theme:
            return HRMSColorTheme.objects.filter(id=theme.theme.id).first()
        else:
...
def get_company(context):
    ...
    cache = getattr(request, "_hrms_company_theme_cache", None)
    if cache is None:
        cache = {}
        request._hrms_company_theme_cache = cache
