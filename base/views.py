...
from hrms.menu import get_settings_menu
from hrms.methods import get_hrms_model_class, remove_dynamic_url
...
def initialize_database_condition():
    ...
    """
    init_database = not HRMSUser.objects.exists()
    if not init_database:
        init_database = True
        superusers = HRMSUser.objects.filter(is_superuser=True)
...
def initialize_database_user(request):
    ...
    if request.method == "POST":
        ...
        user = HRMSUser.objects.filter(username=username).first()
        if user and not hasattr(user, "employee_get"):
            user.delete()
        user = HRMSUser.objects.create_superuser(
...
def login_user(request):
    ...
    if request.method == "POST":
        ...
        if not user:
            user_object = HRMSUser.objects.filter(username=username).first()
...
def _permission_app_label(app_name):
    ...
    Uses AppConfig.verbose_name and drops a leading "HRMS" product prefix.
    """
    import re
    ...
    label = re.sub(r"(?i)^hrms[\s_\-]*", "", label).strip()
...
def group_assign(request):
    ...
    if not group_id:
        return HRMSRedirect(request, message=_("Required parameters are missing"))
    group = Group.objects.filter(id=group_id).first()
    if not group:
        return HRMSRedirect(request, message=_("Group not found"))
...
def group_remove_user(request, uid, gid):
    ...
    user = HRMSUser.objects.filter(id=uid).first()
...
def mail_server_test_email(request):
    ...
    image_path = path.join(settings.STATIC_ROOT, "images/ui/hrms-logo.png")
    company_name = "HRMS"
...
def mail_server_delete(request):
    ...
    if not id:
        return HRMSRedirect(request, message=_("Missing required parameter"))
...
def work_type_request_cancel(request, id):
    ...
    if not work_type_request:
        ...
        return JsonResponse({"result": False}) if is_ajax else HRMSRedirect(request)
...
def update_employee_allocation(request):
    ...
    if not shift:
        return HRMSRedirect(request, message=_("No shift found matching the query."))
...
def shift_request_cancel(request, id):
    ...
    return JsonResponse({"result": True}) if is_ajax else HRMSRedirect(request)
...
def save_date_format(request):
    if request.method == "POST":
        ...
        if selected_format not in settings.HRMS_DATE_FORMATS:
...
@login_required
@permission_required("hrms_audit.change_accountblockunblock")
...
def delete_shift_comment_file(request):
    ...
    if not comment:
        return HRMSRedirect(request, message=_("Comment not found."))
...
def reorder_dashboard_charts(request):
    ...
    return render(
        request,
        "hrms_theme/components/reorder_dashboard_charts.html",
        {"charts": charts},
    )
