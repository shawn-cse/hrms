...
Sections registered:
  ...
  - Appearance : color theme (only when hrms_theme is installed)
...
from hrms.menu import settings_menu
...
def system_preferences_accessibility(request, submenu, user_perms, *args, **kwargs):
    return any(
        ...
        for p in [
            ...
            "hrms_audit.view_accountblockunblock",
            "employee.change_employeegeneralsetting",
            "hrms_audit.view_historytrackingfields",
...
def history_tags_accessibility(request, submenu, user_perms, *args, **kwargs):
    return any(
        ...
        for p in [
            ...
            "hrms_audit.view_audittag",
        ]
    )


def audit_tracking_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("hrms_audit.view_auditmodelconfig")
...
def audit_history_accessibility(request, submenu, user_perms, *args, **kwargs):
    return any(
        ...
        for p in [
            "hrms_audit.view_audittag",
            "hrms_audit.view_auditmodelconfig",
        ]
    )
...
def mail_template_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("base.view_hrmsmailtemplate")


def mail_automation_accessibility(request, submenu, user_perms, *args, **kwargs):
    return apps.is_installed("hrms_automations") and request.user.has_perm(
        "hrms_automations.view_mailautomation"
    )
...
def color_theme_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("hrms_theme.view_hrmscolortheme")


def gdrive_accessibility(request, submenu, user_perms, *args, **kwargs):
    return request.user.has_perm("hrms_backup.view_googledrivebackup")
...
def ldap_accessibility(request, submenu, user_perms, *args, **kwargs):
    return apps.is_installed("hrms_ldap") and any(
        request.user.has_perm(p)
        for p in ["hrms_ldap.add_ldapsettings", "hrms_ldap.update_ldapsettings"]
    )


def google_meet_accessibility(request, submenu, user_perms, *args, **kwargs):
    return apps.is_installed("hrms_meet") and request.user.has_perm(
        "hrms_meet.view_googlecloudcredential"
    )
...
# ---------------------------------------------------------------------------
# 5. Appearance section (only when hrms_theme is installed)
# ---------------------------------------------------------------------------
...
class ThemeManagerSettings:
    title = _("Appearance")
    order = 10
    condition = lambda self, request: apps.is_installed("hrms_theme")
    items = [
        {
            "label": _("Color Theme"),
            "url": reverse_lazy("hrms_theme:color_theme_view"),
