...
def _resolve_checklist_company(request):
    ...
    """
    from hrms.hrms_middlewares import get_selected_company
...
def _exists_for_company(ModelClass, company_id):
    ...
    company.  Mirrors HRMSCompanyManager: a record is visible when it is
...
def _get_setup_checklist_context(request):
    ...
    steps = [
        ...
        },
        {
            ...
            "title": _("Mail Server"),
            "description": _(
                "Configure an outgoing mail server so HRMS can send emails."
            ),
...
def _scoped_active_employee_ids(request):
    ...
    ``None`` means company-scoped (HRMSCompanyManager) — no extra filter.
    """
    user = request.user
