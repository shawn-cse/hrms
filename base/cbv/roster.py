...
from hrms_views.cbv_methods import login_required, paginator_qry
from hrms_views.generic.cbv.views import HRMSCardView, HRMSNavView
...
@method_decorator(login_required, name="dispatch")
class RosterNavView(HRMSNavView):
...
@method_decorator(login_required, name="dispatch")
class RosterGridView(HRMSCardView):
    model = Roster
            ...
            for col_i, d in enumerate(date_range, start=3):
                ...
                if off_reason:
                    ...
                    if off_reason != "Weekly Off Day":
                        c.comment = Comment(f"Holiday: {off_reason}", "HRMS")
                elif d.weekday() >= 5:
                    c.fill = weekend_fill
