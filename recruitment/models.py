...
class Candidate(HRMSModel):
    ...
    def options(self):
        ...
        emp_list = HRMSUser.objects.filter(username__in=mails).values_list(
            "email", flat=True
        )
...
class CandidateDocumentRequest(HRMSModel):
...
class CandidateDocument(HRMSModel):
...
class LinkedInAccount(HRMSModel):
