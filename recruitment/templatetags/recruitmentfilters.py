...
def is_stagemanager(user):
    ...
    """
    try:
        cached = getattr(user, "_hrms_is_stagemanager", None)
        ...
        setattr(user, "_hrms_is_stagemanager", result)
        return result
    except Exception:
        return False
...
def is_any_manager(request):
    ...
    """
    user = request.user
    cached = getattr(user, "_hrms_is_any_manager", None)
    if cached is not None:
        return cached
    ...
    setattr(user, "_hrms_is_any_manager", result)
    return result
...
def is_recruitmentmangers(user):
    ...
    """
    try:
        cached = getattr(user, "_hrms_is_recruitmentmanager", None)
        if cached is not None:
            return cached
        employee_obj = user.employee_get
        result = employee_obj.recruitment_set.exists()
        setattr(user, "_hrms_is_recruitmentmanager", result)
        return result
    except Exception:
        return False
...
def employee(uid):
    ...
    Returns:
        user object
    """
    return HRMSUser.objects.get(id=uid).employee_get if uid is not None else None
