...
from base.models import EmailLog, HRMSMailTemplate, JobPosition, clear_messages
...
def candidate_view(request):
    ...
    # Query the HRMSUser model to check if any email is present
    existing_emails = list(
        HRMSUser.objects.filter(username__in=mails).values_list("email", flat=True)
    )
...
def candidate_conversion(request, cand_id, **kwargs):
    ...
    user_exists = HRMSUser.objects.filter(username=candidate_obj.email).exists()
...
def form_send_mail(request, cand_id=None):
    ...
    HRMSMailTemplate.objects.get_or_create(
        ...
        defaults={
            "body": (
                ...
                '<div style="margin: 18px 0; padding: 16px; background: #f9fafb; border-left: 4px solid #2563eb; border-radius: 8px;">'
