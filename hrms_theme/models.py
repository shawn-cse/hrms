"""
Models for the hrms_theme app
"""

from django.core.exceptions import ValidationError
from django.core.validators import RegexValidator

# Create your hrms_theme models here.
...
THEMES_DATA = [
    {
        "name": "HRMS Blue Theme (Default)",
        "description": "Professional blue HRMS default theme",
        "is_default": True,
        "primary_50": "#eff6ff",
...
class HRMSColorTheme(HRMSModel):
    """
    Model to store predefined color themes for HRMS
    """
    ...
    class Meta:
        """
        Meta option for HRMSColorTheme model
        """
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            if self.is_default:
                query = HRMSColorTheme.objects.filter(is_default=True)
                if self.pk:
                    query = query.exclude(pk=self.pk)
                query.update(is_default=False)
            super().save(*args, **kwargs)
            if self.is_default and self.pk:
                HRMSColorTheme.objects.filter(is_default=True).exclude(
                    pk=self.pk
...
class CompanyTheme(HRMSModel):
    ...
    def get_default_theme(cls):
        ...
        Returns the theme object or None
        """
        return HRMSColorTheme.get_default_theme()
    ...
    def get_theme_for_company(cls, company):
        ...
        # return HRMSColorTheme.objects.filter(
        #     name="Coral Red Theme (Default)"
        # ).first()
        return HRMSColorTheme.objects.filter(is_default=True).first()
