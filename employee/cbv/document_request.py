...
from hrms.decorators import manager_can_enter
from hrms.http.response import HRMSRedirect
from hrms_documents.forms import DocumentForm
from hrms_documents.forms import DocumentRejectCbvForm as RejectForm
from hrms_documents.forms import DocumentRequestForm, DocumentUpdateForm
from hrms_documents.models import Document, DocumentRequest
from hrms_views import models as hrms_views_models
from hrms_views.cbv_methods import (
    hx_request_required,
    login_required,
    saved_filter_path_query,
)
from hrms_views.generic.cbv.pipeline import Pipeline
...
@method_decorator(
    manager_can_enter("hrms_documents.add_documentrequest"), name="dispatch"
)
class DocumentRequestCreateForm(HRMSFormView):
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        self.form = choosesubordinates(
            self.request, self.form, "hrms_documents.add_documentrequest"
        )
    ...
    def form_valid(self, form: DocumentRequestForm) -> HttpResponse:
        if form.is_valid():
            ...
            if refreshed is not None:
                return refreshed
            return HRMSRedirect(self.request)

        return super().form_valid(form)


@method_decorator(login_required, name="dispatch")
class DocumentCreateForm(HRMSFormView):
    """
...
@method_decorator(login_required, name="dispatch")
@method_decorator(manager_can_enter("hrms_documents.add_document"), name="dispatch")
class DocumentRejectCbvForm(HRMSFormView):
    """
...
@method_decorator(login_required, name="dispatch")
class DocumentUploadForm(HRMSFormView):
    """
...
@method_decorator(login_required, name="dispatch")
class DocumentRequestNav(HRMSNavView):
    """
    For nav bar
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.request.user.has_perm(
            ...
            if self.request.user.has_perm(
                "hrms_documents.change_documentrequest"
            ) or is_reportingmanager(self.request):
                self.actions = [
                    {
...
class DocumentRequestPipelineView(Pipeline):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Pipeline (unlike HRMSListView) never builds filter_dict itself,
        ...
        # get_key_instances()-based construction HRMSListView uses.
        ...
        # that every HRMSListView/HRMSCardView gets for free, so there
        ...
        context["stored_filters"] = hrms_views_models.SavedFilter.objects.filter(
...
@method_decorator(login_required, name="dispatch")
class DocumentListView(HRMSListView):
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        # DocumentRequestPipelineView. HRMSListView always populates
        ...
        context["stored_filters"] = hrms_views_models.SavedFilter.objects.none()
        return context
...
class DocumentIndividualTabList(DocumentListView):
    ...
    # HRMSListView table template instead of one of those templates, that
    ...
    # - matching how every other HRMSProfileView sub-tab in this framework
    ...
    def get_queryset(self, queryset=None, filtered=False, *args, **kwargs):
        ...
        # straight to HRMSListView.get_queryset() with our own base
        ...
        return HRMSListView.get_queryset(self, queryset, filtered, *args, **kwargs)
