...
from hrms.signals import post_generic_delete, post_generic_import
from hrms_auth.models import HRMSUser
from hrms_views.cbv_methods import hx_request_required, login_required
from hrms_views.forms import DynamicBulkUpdateForm
...
class EmployeesList(HRMSListView):
    """
    ...
    import_related_model_column_mapping = {
        "employee_user_id": base_models.HRMSUser,
...
@method_decorator(login_required, name="dispatch")
class TabEmployeeWorkList(HRMSListView):
    """
...
@method_decorator(login_required, name="dispatch")
class EmployeeWorkDetails(HRMSDetailedView):
    """
...
@receiver(post_generic_import, sender=HRMSUser)
    ...
    def _set_password(records):
        ...
        if users_to_update:
            ...
            with transaction.atomic():
                HRMSUser.objects.bulk_update(users_to_update, ["password"])
                logger.info(
...
class EmployeeCard(HRMSCardView):
    """
    For card view
    """
