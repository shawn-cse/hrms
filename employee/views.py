...
def allowances_deductions_tab(request, pk):
    ...
    if basic_pay:
        ...
        Allowance = get_hrms_model_class(app_label="payroll", model="allowance")
...
@login_required
@manager_can_enter("hrms_documents.view_documentrequest")
def document_request_view(request):
    ...
    documents = filtersubordinates(
        request=request,
        perm="hrms_documents.view_documentrequest",
        queryset=documents,
    )
...
@login_required
@hx_request_required
@manager_can_enter("hrms_documents.add_documentrequest")
...
@login_required
@hx_request_required
@manager_can_enter("hrms_documents.change_documentrequest")
...
def file_upload(request, id):
    ...
    if not can_access_document(
        request, document_item, "hrms_documents.change_document"
    ):
        return HRMSRedirect(
...
def view_file(request, id):
    ...
    if not can_access_document(
        request, document_obj, "hrms_documents.view_document"
    ):
        return HRMSRedirect(
...
def employee_user_group_assign_delete(_, obj_id):
    ...
    """
    user = HRMSUser.objects.get(id=obj_id)
    user.groups.clear()
...
def employee_account_block_unblock(request, emp_id):
    ...
    if not employee:
        ...
        return redirect(f"{reverse('employee-view')}?view=list")
    user = get_object_or_404(HRMSUser, id=employee.employee_user_id.id)
    if not user:
        messages.info(request, _("Employee not found"))
        return redirect(f"{reverse('employee-view')}?view=list")
...
def employee_view_update(request, obj_id, **kwargs):
    ...
    if (
        ...
        return response
    return HRMSRedirect(request, fallback_url="/employee/employee-view")
...
def employee_delete(request, obj_id):
    ...
    return HRMSRedirect(request, fallback_url=f"/view={view}")
    ...
    if request.method == "POST":
        ...
        for employee_dict in employee_dicts:
            try:
                ...
                existing_user = HRMSUser.objects.filter(username=email).first()
                if existing_user is None:
                    ...
                    user = HRMSUser.objects.create_user(
                        username=email,
                        email=email,
    ...
    for employee in employees:
        for column_value, column_name in selected_columns:
            ...
            if isinstance(value, date):
                try:
                    data = value.strftime(
                        settings.HRMS_DATE_FORMATS.get(date_format, "%Y-%m-%d")
                    )
                except Exception:
                    data = str(value)
...
def employee_history_sidebar(request, pk):
    ...
    same way HRMSModel-based lists open their auto-added History column
    (see generic/history_col.html / hrms_history_view.html).
    """
    employee_obj = Employee.objects.get(id=pk)
    return render(
        request,
