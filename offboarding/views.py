...
def update_stage_order(request, pk):
    ...
    if not offboarding:
        return HRMSRedirect(request, message=_("Offboarding not found"))
...
def delete_employee(request):
    ...
    if instances:
        ...
        notify.send(
            request.user.employee_get,
            recipient=HRMSUser.objects.filter(
...
def add_note(request):
    ...
    if not employee:
        return HRMSRedirect(request, message=_("Employee not found."))
    form = NoteForm()
    if request.method == "POST":
...
def offboarding_note_delete(request, note_id):
    ...
    try:
        ...
        return HRMSRedirect(request, message=_("Note not found."))
...
def update_task_status(request, *args, **kwargs):
    ...
    if not task_id or not status or not stage_id or not employee_ids:
        return HRMSRedirect(request, message=_("Missing required parameters."))
    ...
    if not stage:
        return HRMSRedirect(request, message=_("Stage not found"))
    stage_forms = {}
    stage_forms[str(stage.offboarding_id.id)] = StageSelectForm(
        offboarding=stage.offboarding_id
    )
...
def task_assign(request):
    ...
    if not task:
        return HRMSRedirect(request, message=_("Task not found"))
    for employee in employees:
        try:
...
def request_single_view(request, id):
    ...
    if not letter:
        return HRMSRedirect(request, message=_("Resignation letter not found"))
    context = {
        "letter": letter,
    }
    requests_ids_json = request.GET.get("requests_ids")
...
def resignation_list_swap_response(original_request):
    ...
    Subrequest keeps session (HRMS CACHE filters) without relying on client-side JS reload.
    """
...
def offboarding_dashboard(request):
    ...
    if apps.is_installed("recruitment"):
        Candidate = get_hrms_model_class("recruitment", "candidate")
...
if apps.is_installed("asset"):
    ...
    def dashboard_asset_table(request):
        ...
        """
        AssetAssignment = get_hrms_model_class(
...
if apps.is_installed("pms"):
    ...
    def dashboard_feedback_table(request):
        ...
        Feedback = get_hrms_model_class(app_label="pms", model="feedback")
