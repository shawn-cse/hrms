...
class LeaveTypeCondition(HRMSModel):
    """
...
class LeaveRequest(HRMSModel):
    ...
    def leave_requests_custom_emp_col(self):
        ...
        if apps.is_installed("recruitment"):
            ...
            if request is not None:
                cache = getattr(request, "_hrms_interview_clash_cache", None)
                if cache is None:
                    cache = request._hrms_interview_clash_cache = {}
    ...
    def is_approved(self):
        request = getattr(hrms_middlewares._thread_locals, "request", None)
        if request:
            if not hasattr(request.user, "_hrms_employee_cache"):
                request.user._hrms_employee_cache = Employee.objects.filter(
                    employee_user_id=request.user
                ).first()
            employee = request.user._hrms_employee_cache
