...
def employee_leave_details(request):
    ...
    if not employee:
        return HRMSRedirect(request, message=_("No leave found matching the query."))
...
def create_leaverequest_comment(request, leave_id):
    ...
    if not leave:
        return HRMSRedirect(request, message=_("Leave request not found."))
