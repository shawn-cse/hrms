...
from hrms.methods import remove_dynamic_url
...
def validate_bulk_attendance(request):
    ...
    for obj_id in filtered_ids:
        try:
            ...
            # Historical upstream issue reference: #1055
            if (
...
def create_attendancerequest_comment(request, attendance_id):
    ...
    if not attendance:
        return HRMSRedirect(request, message=_("Attendance not found."))
...
def work_record_export(request):
    ...
    format_string = settings.HRMS_DATE_FORMATS.get(date_format)
