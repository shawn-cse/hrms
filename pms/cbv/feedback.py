...
@method_decorator(login_required, name="dispatch")
class FeedbackListView(HRMSListView):
    """
    list view
    """
...
@method_decorator(login_required, name="dispatch")
class FeedbackGenericTabView(HRMSTabView):
    """
...
@method_decorator(login_required, name="dispatch")
class AnonymousFeedbackTab(HRMSListView):
    """
...
class _FeedbackTabNavBase(HRMSNavView):
    """
...
@method_decorator(login_required, name="dispatch")
class AddAnonymousFeedbackForm(HRMSFormView):
    """
    form view
    """
        ...
        if form.is_valid():
            if form.instance.pk:
                ...
                if feedback.based_on == "employee":
                    notify.send(
                        HRMSUser.objects.filter(username="HRMS Bot").first(),
...
class PerformanceTab(SelfFeedbacktab):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # HRMSListView defaults selected_instances_key_id to the shared
...
class FeedbackUpdateFormView(HRMSFormView):
    """
...
@method_decorator(login_required, name="dispatch")
class AnounyFeedbackDetailView(HRMSDetailedView):
