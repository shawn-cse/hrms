...
from base.forms import ModelForm as HRMSModelForm
...
class FeedbackForm(HRMSModelForm):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # # HRMS multi-select filter for subordinates
...
class BonusPointSettingForm(HRMSModelForm):
    """
...
class EmployeeBonusPointForm(HRMSModelForm):
    """
...
class EmployeeFeedbackForm(HRMSModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["others_id"] = HRMSMultiSelectField(
            queryset=Employee.objects.filter(employee_work_info__isnull=False),
            widget=HRMSMultiSelectWidget(
                filter_route_name="employee-widget-filter",
                filter_class=EmployeeFilter,
    ...
    def clean(self):
        cleaned_data = super().clean()
        if isinstance(self.fields["others_id"], HRMSMultiSelectField):
...
class BulkFeedbackForm(HRMSModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["employee_ids"] = HRMSMultiSelectField(
            queryset=Employee.objects.filter(employee_work_info__isnull=False),
            widget=HRMSMultiSelectWidget(
                filter_route_name="employee-widget-filter",
                filter_class=EmployeeFilter,
    ...
    def clean(self):
        cleaned_data = super().clean()
        if isinstance(self.fields["employee_ids"], HRMSMultiSelectField):
