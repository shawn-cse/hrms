...
from hrms_meet.filters import GoogleMeetingFilter
from hrms_meet.form import GoogleCloudCredentialForm, GoogleMeetingForm
from hrms_meet.models import GoogleCloudCredential, GoogleCredential, GoogleMeeting
...
@method_decorator(
    permission_required("hrms_meet.view_googlecloudcredential"), name="dispatch"
)
class GMeetCredentialSectionView(views.HRMSSectionView):
    """
    View for displaying the Google Meet credentials section.

    Inherits from `HRMSSectionView` to manage the section for Google Cloud credentials in the user interface.
...
@method_decorator(login_required, name="dispatch")
@method_decorator(check_integration_enabled(app_name="hrms_meet"), name="dispatch")
@method_decorator(
    permission_required("hrms_meet.view_googlecloudcredential"), name="dispatch"
)
class GmeetCredentialNavView(views.HRMSNavView):
    """
    View for the Google Meet credentials nav view.

    Inherits from `HRMSNavView` to manage nav view for Google Cloud credentials. Handles dynamic attributes
...
class GmeetCredentialListView(views.HRMSListView):
    """
    View for listing Google Meet credentials.

    Inherits from `HRMSListView` to manage the listing of Google Cloud credentials with actions like edit and delete.
...
@method_decorator(check_integration_enabled(app_name="hrms_meet"), name="dispatch")
@method_decorator(
    permission_required("hrms_meet.add_googlecloudcredential"), name="dispatch"
)
class GoogleCredentialsFormView(views.HRMSFormView):
    """
    View for creating or updating Google Cloud credentials.

    Inherits from `HRMSFormView` to manage the form for creating or updating Google Cloud credentials.
...
@method_decorator(login_required, name="dispatch")
@method_decorator(
    permission_required("hrms_meet.add_googlemeeting"), name="dispatch"
)
@method_decorator(check_integration_enabled(app_name="hrms_meet"), name="dispatch")
class GmeetFormView(views.HRMSFormView):
    """
    View for creating or updating Google Meet meetings.

    Inherits from `HRMSFormView` to manage the form for creating or updating Google Meet meetings.
    Handles form validation, success, and failure responses. Displays success or error messages accordingly.
...
@method_decorator(check_integration_enabled(app_name="hrms_meet"), name="dispatch")
@method_decorator(
    permission_required("hrms_meet.view_googlemeeting"), name="dispatch"
)
class GmeetSectionView(views.HRMSSectionView):
    """
    View for displaying the Google Meet section.

    Inherits from `HRMSSectionView` to manage the section for Google Meet in the user interface.
...
class GmeetNavView(views.HRMSNavView):
    """
    Nav view for Google Meet.

    Inherits from `HRMSNavView` to manage nav bar for Google Meet view. Handles dynamic attributes for creating
...
class GmeetListView(views.HRMSListView):
    """
    View for listing Google Meetings.

    Inherits from `HRMSListView` to manage the listing of Google meetings with actions like edit, delete, and view meeting link.
...
class GmeetDetailedView(views.HRMSDetailedView):
    """
    Detailed view for Google Meetings.

    Inherits from `HRMSDetailedView` to manage the detailed view of a specific Google Meeting.
