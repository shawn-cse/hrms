# HRMS Tour & Getting Started Checklist

Functional and technical reference for HRMS’s onboarding UX:

1. **Product Tours** (`hrms_tour`) — guided, driver.js walkthroughs on any page
2. **Setup Checklist** — “Get started with HRMS” banner on the dashboard (8 setup steps)
...
## Part 1 — Product Tours (`hrms_tour`)
...
1. Authenticated page load injects `window.HRMS_TOUR` and `tourController.js`
...
│    window.HRMS_TOUR = { page, path, activeUrl, … }       │
...
└──────────────────────────┬──────────────────────────────────┘
                           │
┌──────────────────────────▼──────────────────────────────────┐
│  Models (HRMSModel + HRMSCompanyManager)              │
...
- Installed app: `hrms_tour` (`hrms/settings/base.py`)
- URLs: `path("", include("hrms_tour.urls"))` in `hrms/urls.py`
- Context processor: `hrms_tour.context_processors.pending_tours_flag`
  - Exposes `tour_launcher_enabled`, `tour_has_pending` for the header Help button
- Settings menu: `hrms_tour/sidebar.py` → “Product Tours / Manage Tours”
- Scripts: `hrms_theme/.../footer_scripts.html` (authenticated only)
...
| `tour-nav` / `tour-list` | HRMSNavView / HRMSListView | `view_tour` |
| `tour-create-form` / `tour-update-form` | HRMSFormView | `add_tour` / change via update |
| `tour-steps` / `tour-step-form` / `tour-step-delete` | HTMX panel | `view_tour` / `change_tour` |

Patterns: HRMSListView row actions, HTMX modals, generic delete.

#### Frontend (`tourController.js`)

- Depends on global **driver.js** (`window.driver.js.driver`)
- Reads `window.HRMS_TOUR`
- Builds driver steps from API payload
- On destroy: posts `completed` if last step reached, else `skipped`
- Exposes launcher UI (`#hrmsTourLauncher`) and `window.hrmsTour` for Start/Replay
- Header button `#tourLauncherBtn` toggles the panel

#### Permissions

Django model perms on `hrms_tour.Tour`:
...
## Part 2 — Setup Checklist (“Get started with HRMS”)

Dashboard onboarding banner that walks admins through foundational HR configuration. Not part of `hrms_tour`, but the primary “first run” tour of the product.

### Functional overview

**Copy & UX**

- Title: **Get started with HRMS**
...
`_exists_for_company` mirrors `HRMSCompanyManager`: record counts if assigned to company **or** company FK/M2M is null (shared lookup data).
...
- Brand coral gradient (`#2563eb` / `#e8705e`)
...
hrms_theme/.../footer_scripts.html   HRMS_TOUR bootstrap
hrms_theme/.../header.html           Take a tour button
```
...
6. **Permissions** — grant `hrms_tour.view_tour` (and change/add) only to admins who should author tours.
`)
