...
2. Settings admin CRUD (permission-gated) to author tours and their ordered
   steps, following the HRMSSectionView + HRMSNavView + HRMSListView +
   HRMSFormView pattern used elsewhere in Settings.
"""
...
from hrms.decorators import permission_required
from hrms_tour.filters import TourFilter
from hrms_tour.forms import TourForm, TourStepForm
from hrms_tour.models import Tour, TourProgress, TourStep
...
@auth_login_required
@permission_required("hrms_tour.view_tour")
def tour_settings_page(request):
    """Render the Tour Management page inside the Settings shell."""
    return render(request, "hrms_tour/tour_settings.html")


@method_decorator(login_required, name="dispatch")
@method_decorator(permission_required("hrms_tour.view_tour"), name="dispatch")
class TourNav(HRMSNavView):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.request.user.has_perm("hrms_tour.add_tour"):
            self.create_attrs = f"""
...
class TourList(HRMSListView):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        if self.request.user.has_perm("hrms_tour.change_tour"):
            self.actions.insert(
                0,
                {
                    "action": _("Edit"),
        ...
        if self.request.user.has_perm("hrms_tour.delete_tour"):
            self.actions.append(
                {
                    "action": _("Delete"),
                    "icon": "trash-outline",
                    "attrs": """
                        ...
                        hx-get="{get_delete_url}?model=hrms_tour.tour&pk={pk}"
                        data-toggle="oh-modal-toggle"
...
@method_decorator(login_required, name="dispatch")
@method_decorator(permission_required("hrms_tour.add_tour"), name="dispatch")
class TourFormView(HRMSFormView):
...
def _render_steps_panel(request, tour):
    return render(
        request,
        "hrms_tour/steps_panel.html",
        {
            "tour": tour,
            "steps": tour.steps.all(),
            "can_edit": request.user.has_perm("hrms_tour.change_tour"),
        },
    )
...
@auth_login_required
@permission_required("hrms_tour.change_tour")
