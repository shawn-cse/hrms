...
Workbook layout (aligned with HRMS explorer exports):
...
# ---------------------------------------------------------------------------
# Style tokens — HRMS primary (coral / #2563eb)
# ---------------------------------------------------------------------------
...
def _currency_format() -> tuple[str, str]:
    ...
    base/templatetags/hrmsfilters.py::currency_symbol_position, with the
...
def _apply_print_setup(ws, meta: Optional[dict] = None, landscape: bool = False):
    ...
    product = meta.get("product_name") or "HRMS"
...
def _write_letterhead(ws, company: dict, styles, meta: dict, col_span: int = 6) -> int:
    ...
    product = ws.cell(
        row=1,
        column=col_span,
        value=meta.get("product_name") or "HRMS · Standard Reports",
    )
...
def _write_cover(wb, payload: dict[str, Any], meta: Optional[dict] = None):
    ...
    footer = ws.cell(
        ...
        value=(
            ...
            "Figures are aggregated server-side from HRMS source data."
        ),
    )
...
_CHART_PALETTE_HEX = ["2563EB", "0EA5E9", "15803D", "7C3AED", "0D9488", "F59E0B"]
...
def export_xlsx(
    ...
    props = wb.properties
    props.title = str(payload.get("title") or "HRMS Report")
    company = _company_from_meta(meta)
    props.creator = company.get("name") or "HRMS"
    props.description = (
