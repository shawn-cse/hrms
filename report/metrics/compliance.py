...
def audit_activity(filters: ReportFilters) -> dict:
    """Recent audit log activity summary when hrms_audit / auditlog is available."""
    try:
...
def document_expiry(filters: ReportFilters) -> dict:
    """Expiring employee documents (hrms_documents) and optional assets."""
    rows = []
    kpis_docs = 0

    if apps.is_installed("hrms_documents"):
        try:
            Document = apps.get_model("hrms_documents", "Document")
            ...
            for obj in qs.select_related("employee_id")[:100]:
                ...
                rows.append(
                    {
                        "source": "hrms_documents.Document",
                        "title": str(obj),
