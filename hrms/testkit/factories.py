"""Lightweight model factories for HRMS TestCase suites."""
...
def make_user(
    ...
    *,
    ...
    email = email or f"{username}@test.hrms"
    if is_superuser:
        return HRMSUser.objects.create_superuser(
            username=username, email=email, password=password
        )
    return HRMSUser.objects.create_user(
        username=username, email=email, password=password
    )
...
def make_employee(
    *,
    ...
    Employee.save() auto-creates a HRMSUser when none is set; pass
