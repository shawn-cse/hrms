...
Applied from base settings when DEBUG is False or HRMS_ENV=production.
...
def is_production_mode(debug: bool, hrms_env: str) -> bool:
    return (not debug) or (hrms_env or "").strip().lower() == "production"
...
def validate_production_secrets(secret_key, allowed_hosts, db_init_password):
    ...
    elif secret_key.startswith("django-insecure-") or secret_key.lower().startswith(
        ("change-me", "change_me")
    ):
        errors.append(
    ...
    if (
        not db_init_password
        or db_init_password in INSECURE_DB_INIT_PASSWORDS
        or str(db_init_password).lower().startswith(("change-me", "change_me"))
    ):
        errors.append(
    ...
    if errors:
        raise ImproperlyConfigured(
            "HRMS production security check failed:\n- " + "\n- ".join(errors)
        )
