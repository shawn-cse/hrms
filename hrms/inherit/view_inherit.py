"""
hrms/inherit/view_inherit.py

HRMSViewInheritMixin — add to an extension class to enable view replacement.

Two _inherit modes
------------------

Mode 1  _inherit = True   (direct subclass replacement)
  Include HRMSViewInheritMixin as the first base alongside the target view.
  ...
  Full Python inheritance — super(), MRO, and attribute lookup all work normally.
    ...
    from hrms.inherit import HRMSViewInheritMixin

    class AssetListExtension(HRMSViewInheritMixin, AssetListView):
        _inherit = True
...
Mode 2  _inherit = "app_label.ClassName"  (indirect attribute injection)
  ...
  The target view must be registered via register_view() to be reachable.
    ...
    class AssetListExtension(HRMSViewInheritMixin):
        _inherit = "asset.AssetList"
        columns = ["name", "asset_status", "warranty_expiry_date"]
"""

from hrms.inherit.view_registry import register_extension, register_replacement
...
class HRMSViewInheritMixin:
    """
    Mixin that enables _inherit on HRMS CBV extension classes.
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if inherit is True:
            parent = next(
                (b for b in cls.__bases__ if b is not HRMSViewInheritMixin),
                None,
            )
